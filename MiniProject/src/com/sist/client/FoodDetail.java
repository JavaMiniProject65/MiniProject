...
public class FoodDetail extends JPanel implements ActionListener
{
    ...
    public FoodDetail(ControllerPanel cp)
    {
    	...
    	String[] temp={"업체명","주소","전화","음식종류","영업시간",
    			  "주차","평점","가격대","테마"};
    	mainLa.setBounds(350, 35, 400, 500);
    	add(mainLa);
    	for(int i=0;i<las.length;i++)
    	{
    		las[i]=new JLabel(temp[i]);
    		las[i].setBounds(850, i*40+80, 150, 30);
    		add(las[i]);
    		lap[i]=new JLabel("");
    		lap[i].setBounds(920, i*40+80,200, 30);
    		add(lap[i]);
    	}
    	ta=new JTextPane();
    	ta.setEnabled(false);
    	JScrollPane js=new JScrollPane(ta);
    	js.setBounds(350, 550, 850, 150);
    	add(js);
	...
	public void print(int fno)
	{
		...
		try
		{
			...
			Image img=ImageChange.getImage(new ImageIcon(url), 400, 470);
